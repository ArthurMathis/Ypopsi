...
class InscriptBLc(TestCandidates):
    # * CONSTRUCTOR * #
    def __init__(self):
        """
        Initialise le test avec un nom descriptif
        """
        super().__init__("Test d'inscription de la BL c (candidat nécessaire)")
    ...
    def run(self):
        ...
        try:
            ...
            valid = self.getBL(driver, [ self.APP_CANDIDATES_BL_C ])
            valid = valid and self.getBL(driver, [ self.APP_CANDIDATES_BL_A, self.APP_CANDIDATES_BL_B ], False)

...
class User {
     ...
     * 
     * @var Int The primary key of establishment 
     */
    private $establishment;
    /**
     * Private attribute containing the user's role
     * 
     * @var Int The primary key of his role
     */
    private $role;
    /**
    ...
    private function setEstablishment(int $establishment) {
        if($establishment < 0)  
            throw new InvalideUtilisateurExceptions("La clé d'établissement ne peut pas être négative...");
        else 
    ...
    private function setRole(int $role) {
        if($role < 0)  
            throw new InvalideUtilisateurExceptions("La clé de rôle ne peut pas être négative...");
        else 
            $this->role = $role;
    }
    /**
     ...
     * @return User
     */
    static public function makeUser(array $data): ?User {
        if(empty($data))

...
class Moment {
     ...
     *  Class constructor
     * @param Int $timestamp
     * @throws MomentExceptions If the timestamp is invalid
     */
    ...
    private function setDate($timestamp){
        if(!Moment::isTimestamp($timestamp))
            throw new MomentExceptions("Impossible de récupérer la date sans timestamp !");
     ...
     * @throws MomentExceptions If the moment's timestamp is invalid
     ...
     * @throws MomentExceptions If the date or the hour is invalid
     * @return Moment|NULL 
     */
    public static function fromDate($date, $hour = '12:00:00'): ?Moment {
        if(!Moment::isDate($date) || !Moment::isHour($hour))
            throw new MomentExceptions('Impossible de générer le moment sans date');
        
        return new Moment((new DateTime($date . ' ' . $hour, new DateTimeZone('Europe/Paris')))->getTimestamp());
    }
    /**
     ...
     * @throws MomentExceptions If the MomentExceptions are invalids
    ...
    public static function getTimestampFromDate($date, $hour = '12:00:00'): ?String {
        ...
        return date('Y-m-d H:i:s', (new DateTime($date . ' ' . $hour, new DateTimeZone('Europe/Paris')))->getTimestamp());
    }
}